...
$executiveTeam = $db->fetchAll(
    "SELECT * FROM team_members WHERE is_active = 1 AND is_executive = 1 ORDER BY display_order ASC LIMIT 4"
);
                ...
                <div class="bg-light-navy/50 backdrop-blur-sm rounded-lg p-6 border border-slate-800 hover:border-accent/30 transition text-center" data-aos="fade-up" data-aos-delay="<?= $index * 100 ?>">
                    <div class="w-24 h-24 mx-auto rounded-full overflow-hidden mb-4 border-2 border-accent/30">
                        <?php if ($member['image_path']): ?>
                            <img src="uploads/team/<?= htmlspecialchars($member['image_path']) ?>" alt="<?= htmlspecialchars($member['name']) ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                            <div class="w-full h-full bg-gradient-to-br from-accent/20 to-secondary-accent/20 flex items-center justify-center">
                    ...
                    <div class="flex justify-center gap-3 mt-4">
                        <?php if ($member['linkedin_url']): ?>
                            <a href="<?= htmlspecialchars($member['linkedin_url']) ?>" target="_blank" class="text-slate-400 hover:text-accent transition">
                        ...
                        <?php if ($member['github_url']): ?>
                            <a href="<?= htmlspecialchars($member['github_url']) ?>" target="_blank" class="text-slate-400 hover:text-accent transition">
